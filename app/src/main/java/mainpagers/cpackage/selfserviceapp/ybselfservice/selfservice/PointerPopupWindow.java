...
public class PointerPopupWindow extends PopupWindow {
    ...
    public PointerPopupWindow(int height) {
        this(APP.mAppApplication, ViewGroup.LayoutParams.WRAP_CONTENT,height);
    }
    ...
    public void showAsPointer(View anchor, int xoff, int yoff) {
        ...
        if(isShowUp){
            showAtLocation(anchor, Gravity.LEFT | Gravity.TOP, loc[0] + xoff, loc[1] - getHeight() + yoff);
        }else{
            showAtLocation(anchor, Gravity.LEFT | Gravity.BOTTOM,loc[0] + xoff, loc[1] - getHeight() + yoff);
        }
